package com.whpe.bean;

import java.math.BigDecimal;
import java.util.Date;

public class TCardInfo {
    private String fxkh;

    private String csdm;

    private String hydm;

    private String klx;

    private String mklx;

    private String zklx;

    private String csn;

    private String kzhzt;

    private Date sksj;

    private Long yj;

    private Date czsj;

    private BigDecimal czje;

    private Date sysj;

    private BigDecimal ye;

    private Long sycs;

    private Date tksj;

    private Long tkyj;

    private BigDecimal tkje;

    private Long tkcs;

    private Date gssj;

    private Long gsrybh;

    private Date jgsj;

    private Long jgrybh;

    private String gscq;

    private String changefxkh;

    private Date changesj;

    private Long changerybh;

    private String hmdbz;

    private Long cardcnt;

    private Long posseq;

    private Long rybh;

    private String xm;

    private String xb;

    private String zjlx;

    private String zjbh;

    private Date cshrq;

    private Date qyrq;

    private Date yxrq;

    private Date nsrq;

    private Long nsrybh;

    private String isns;

    private BigDecimal kzhxfjesum;

    private BigDecimal kzhczjesum;

    private Long fkrybh;

    private String fkxm;

    private String samkh;

    private String ykbz;

    private String jy;

    private String sqbz;

    private String bz;

    private String pin;

    private String zcflag;

    private Long zcrybh;

    public String getFxkh() {
        return fxkh;
    }

    public void setFxkh(String fxkh) {
        this.fxkh = fxkh == null ? null : fxkh.trim();
    }

    public String getCsdm() {
        return csdm;
    }

    public void setCsdm(String csdm) {
        this.csdm = csdm == null ? null : csdm.trim();
    }

    public String getHydm() {
        return hydm;
    }

    public void setHydm(String hydm) {
        this.hydm = hydm == null ? null : hydm.trim();
    }

    public String getKlx() {
        return klx;
    }

    public void setKlx(String klx) {
        this.klx = klx == null ? null : klx.trim();
    }

    public String getMklx() {
        return mklx;
    }

    public void setMklx(String mklx) {
        this.mklx = mklx == null ? null : mklx.trim();
    }

    public String getZklx() {
        return zklx;
    }

    public void setZklx(String zklx) {
        this.zklx = zklx == null ? null : zklx.trim();
    }

    public String getCsn() {
        return csn;
    }

    public void setCsn(String csn) {
        this.csn = csn == null ? null : csn.trim();
    }

    public String getKzhzt() {
        return kzhzt;
    }

    public void setKzhzt(String kzhzt) {
        this.kzhzt = kzhzt == null ? null : kzhzt.trim();
    }

    public Date getSksj() {
        return sksj;
    }

    public void setSksj(Date sksj) {
        this.sksj = sksj;
    }

    public Long getYj() {
        return yj;
    }

    public void setYj(Long yj) {
        this.yj = yj;
    }

    public Date getCzsj() {
        return czsj;
    }

    public void setCzsj(Date czsj) {
        this.czsj = czsj;
    }

    public BigDecimal getCzje() {
        return czje;
    }

    public void setCzje(BigDecimal czje) {
        this.czje = czje;
    }

    public Date getSysj() {
        return sysj;
    }

    public void setSysj(Date sysj) {
        this.sysj = sysj;
    }

    public BigDecimal getYe() {
        return ye;
    }

    public void setYe(BigDecimal ye) {
        this.ye = ye;
    }

    public Long getSycs() {
        return sycs;
    }

    public void setSycs(Long sycs) {
        this.sycs = sycs;
    }

    public Date getTksj() {
        return tksj;
    }

    public void setTksj(Date tksj) {
        this.tksj = tksj;
    }

    public Long getTkyj() {
        return tkyj;
    }

    public void setTkyj(Long tkyj) {
        this.tkyj = tkyj;
    }

    public BigDecimal getTkje() {
        return tkje;
    }

    public void setTkje(BigDecimal tkje) {
        this.tkje = tkje;
    }

    public Long getTkcs() {
        return tkcs;
    }

    public void setTkcs(Long tkcs) {
        this.tkcs = tkcs;
    }

    public Date getGssj() {
        return gssj;
    }

    public void setGssj(Date gssj) {
        this.gssj = gssj;
    }

    public Long getGsrybh() {
        return gsrybh;
    }

    public void setGsrybh(Long gsrybh) {
        this.gsrybh = gsrybh;
    }

    public Date getJgsj() {
        return jgsj;
    }

    public void setJgsj(Date jgsj) {
        this.jgsj = jgsj;
    }

    public Long getJgrybh() {
        return jgrybh;
    }

    public void setJgrybh(Long jgrybh) {
        this.jgrybh = jgrybh;
    }

    public String getGscq() {
        return gscq;
    }

    public void setGscq(String gscq) {
        this.gscq = gscq == null ? null : gscq.trim();
    }

    public String getChangefxkh() {
        return changefxkh;
    }

    public void setChangefxkh(String changefxkh) {
        this.changefxkh = changefxkh == null ? null : changefxkh.trim();
    }

    public Date getChangesj() {
        return changesj;
    }

    public void setChangesj(Date changesj) {
        this.changesj = changesj;
    }

    public Long getChangerybh() {
        return changerybh;
    }

    public void setChangerybh(Long changerybh) {
        this.changerybh = changerybh;
    }

    public String getHmdbz() {
        return hmdbz;
    }

    public void setHmdbz(String hmdbz) {
        this.hmdbz = hmdbz == null ? null : hmdbz.trim();
    }

    public Long getCardcnt() {
        return cardcnt;
    }

    public void setCardcnt(Long cardcnt) {
        this.cardcnt = cardcnt;
    }

    public Long getPosseq() {
        return posseq;
    }

    public void setPosseq(Long posseq) {
        this.posseq = posseq;
    }

    public Long getRybh() {
        return rybh;
    }

    public void setRybh(Long rybh) {
        this.rybh = rybh;
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getZjlx() {
        return zjlx;
    }

    public void setZjlx(String zjlx) {
        this.zjlx = zjlx == null ? null : zjlx.trim();
    }

    public String getZjbh() {
        return zjbh;
    }

    public void setZjbh(String zjbh) {
        this.zjbh = zjbh == null ? null : zjbh.trim();
    }

    public Date getCshrq() {
        return cshrq;
    }

    public void setCshrq(Date cshrq) {
        this.cshrq = cshrq;
    }

    public Date getQyrq() {
        return qyrq;
    }

    public void setQyrq(Date qyrq) {
        this.qyrq = qyrq;
    }

    public Date getYxrq() {
        return yxrq;
    }

    public void setYxrq(Date yxrq) {
        this.yxrq = yxrq;
    }

    public Date getNsrq() {
        return nsrq;
    }

    public void setNsrq(Date nsrq) {
        this.nsrq = nsrq;
    }

    public Long getNsrybh() {
        return nsrybh;
    }

    public void setNsrybh(Long nsrybh) {
        this.nsrybh = nsrybh;
    }

    public String getIsns() {
        return isns;
    }

    public void setIsns(String isns) {
        this.isns = isns == null ? null : isns.trim();
    }

    public BigDecimal getKzhxfjesum() {
        return kzhxfjesum;
    }

    public void setKzhxfjesum(BigDecimal kzhxfjesum) {
        this.kzhxfjesum = kzhxfjesum;
    }

    public BigDecimal getKzhczjesum() {
        return kzhczjesum;
    }

    public void setKzhczjesum(BigDecimal kzhczjesum) {
        this.kzhczjesum = kzhczjesum;
    }

    public Long getFkrybh() {
        return fkrybh;
    }

    public void setFkrybh(Long fkrybh) {
        this.fkrybh = fkrybh;
    }

    public String getFkxm() {
        return fkxm;
    }

    public void setFkxm(String fkxm) {
        this.fkxm = fkxm == null ? null : fkxm.trim();
    }

    public String getSamkh() {
        return samkh;
    }

    public void setSamkh(String samkh) {
        this.samkh = samkh == null ? null : samkh.trim();
    }

    public String getYkbz() {
        return ykbz;
    }

    public void setYkbz(String ykbz) {
        this.ykbz = ykbz == null ? null : ykbz.trim();
    }

    public String getJy() {
        return jy;
    }

    public void setJy(String jy) {
        this.jy = jy == null ? null : jy.trim();
    }

    public String getSqbz() {
        return sqbz;
    }

    public void setSqbz(String sqbz) {
        this.sqbz = sqbz == null ? null : sqbz.trim();
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz == null ? null : bz.trim();
    }

    public String getPin() {
        return pin;
    }

    public void setPin(String pin) {
        this.pin = pin == null ? null : pin.trim();
    }

    public String getZcflag() {
        return zcflag;
    }

    public void setZcflag(String zcflag) {
        this.zcflag = zcflag == null ? null : zcflag.trim();
    }

    public Long getZcrybh() {
        return zcrybh;
    }

    public void setZcrybh(Long zcrybh) {
        this.zcrybh = zcrybh;
    }
}